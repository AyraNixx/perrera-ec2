...
class Constants 
{
    ...
    const GET_ANIMALES_INACTIVE = 'SELECT a.*, e.nombre as nombre_especie FROM perrera.animales a INNER JOIN especies e ON a.especies_id = e.id WHERE a.disponible = 0 AND e.disponible = 1';
    ...
    const GET_ANIMAL = "SELECT a.*, e.nombre as especie, j.ubicacion as jaula, j.id as jaula_id, GROUP_CONCAT(i.id SEPARATOR ','), GROUP_CONCAT(i.animales_id SEPARATOR ','), GROUP_CONCAT(i.ruta SEPARATOR ','), GROUP_CONCAT(i.disponible SEPARATOR ',') FROM perrera.animales a INNER JOIN perrera.especies e ON a.especies_id = e.id LEFT JOIN perrera.jaulas j ON a.jaulas_id = j.id LEFT JOIN perrera.imgs i ON a.id = i.animales_id WHERE a.id = :id GROUP BY a.id, a.nombre, a.especies_id, a.raza, a.genero, a.tamanio, a.peso, a.disponible, a.colores, a.personalidad, a.fech_nac, a.estado_adopcion, a.estado_salud, a.adoptante_id, a.necesidades_especiales, a.otras_observaciones, a.jaulas_id, e.id, e.nombre, j.id, j.ubicacion, a.fecha_adopcion";
    const GET_ANIMAL2 = "SELECT a.id, a.nombre, a.especies_id, a.raza, a.genero, a.tamanio, a.peso, a.colores, a.personalidad, a.fech_nac, a.estado_adopcion, a.estado_salud, a.necesidades_especiales, a.otras_observaciones, a.jaulas_id, e.nombre as especie, j.ubicacion as jaula, j.id as jaula_id FROM perrera.animales a INNER JOIN perrera.especies e ON a.especies_id = e.id LEFT JOIN perrera.jaulas j ON a.jaulas_id = j.id WHERE a.id = :id";
    ...
    const GET_TAREAS_EMPLEADO_LOGIN = 'SELECT ta.id, ta.asunto, t.descripcion, ta.empleados_id, ta.fecha_asignacion, ta.fecha_finalizacion, ta.estado_asignacion, ta.prioridad FROM tareas_asignadas ta JOIN tareas t ON ta.tareas_id1 = t.id WHERE ta.empleados_id = (SELECT id FROM empleados WHERE correo = :correo) LIMIT 15';
    const GET_TAREAS_EMPLEADO_TERMINADAS = 'SELECT ta.id, ta.asunto, t.descripcion, ta.empleados_id, ta.fecha_asignacion, ta.fecha_finalizacion, ta.estado_asignacion, ta.prioridad FROM tareas_asignadas ta JOIN tareas t ON ta.tareas_id1 = t.id WHERE ta.empleados_id = (SELECT id FROM empleados WHERE correo = :id) AND estado_asignacion IN ("Completada", "Cancelada", "Archivada")';
    const GET_TAREAS_EMPLEADO = 'SELECT ta.id, ta.asunto, t.descripcion, ta.empleados_id, ta.fecha_asignacion, ta.fecha_finalizacion, ta.estado_asignacion, ta.prioridad FROM tareas_asignadas ta JOIN tareas t ON ta.tareas_id1 = t.id WHERE ta.empleados_id = (SELECT id FROM empleados WHERE correo = :id)';
