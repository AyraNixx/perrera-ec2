    ...
    <table id="list-container" class="table table-striped mb-0" style="vertical-align:middle">
        ...
        <tbody>
            ...
            foreach ($data as $show_data) {
                ...
                echo "<td>" . (($show_data["ocupada"] == '0') ? 'SI' : 'NO') . "</td>";
                ...
                <div class="btn-group dropdown d-block" style="position:relative">
                    ...
                    <div class="btn-dropdown-options w-auto position-absolute start-0">
                        <ul class="list-unstyled m-0">
                            <li>
                                <a href=<?= "../controllers/JaulaC.php?action=show_register&id=" . $show_data["id"] ?>>Ver</a>
                            </li>
                            <li>
                                <a href=<?= "../controllers/JaulaC.php?action=update_modal&id=" . $show_data["id"] ?>>Editar</a>
                            </li>
                            <li>
                                <a href=<?= "../controllers/JaulaC.php?action=sdelete&id=" . $show_data["id"] ?>>Borrar</a>
                            </li>
                            <?php
                            if ($_SESSION["rol"] == Constants::ROL_ADMIN && $show_data["disponible"] == '0') {
                            ?>
                                <li>
                                    <a href=<?= "../controllers/JaulaC.php?action=undelete&id=" . $show_data["id"] ?>>Recuperar registro</a>
                                </li>
                            <?php } ?>
                        </ul>
                    </div>
